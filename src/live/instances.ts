...
import { ActivityAvailability, ActivityCost, activityCostCalculator, AttackActivityTarget, BuildingActivityTarget, ClaimActivityTarget, DismantlingActivityTarget, ExplorationActivityTarget, ResearchActivityTarget, SpyActivityTarget } from '../models/activities'
...
export class LiveGameInstance {
    /** Mapa de elementos del juego */
    private gamedex: GameData;
    /** Fecha de la ultima comprobación de la cola */
    private lastQueueCheck: number;
    /** Descriptor del timer de la cola de actividad */
    private queueInterval: NodeJS.Timer;
    /** Descriptor del timer de la recolección de recursos */
    private resourceInterval: NodeJS.Timer;
    /** Fecha de inicio de la instancia */
    private startDate: number;
    /** Assets empleados en el juego */
    private assets: Asset[] = [];
    /** Cache de props de los jugadores, para notificar cambios */
    private propCache:Record<string,Properties>;
    ...
    constructor(private instance: GameInstance, private game: Game) {
        ...
        this.instance.players.forEach( player => {
            this.propCache[player.playerId] = this.calculatePlayerProperties(player,{placeables:true,technologies:true,cells:true});
        })
    ...
    getActivityCost(player: InstancePlayer, type: ActivityType, target?: ActivityTarget): ActivityCost {
        const activity = this.gamedex.activities.get(type);
        const props = this.calculatePlayerProperties(player, { cells: true, placeables: true, technologies: true });
        const cost = activityCostCalculator(props, this.gamedex, type, target);
        return cost;
    }
    ...
    private checkActivityAvailability(player: InstancePlayer, type: ActivityType, target?: ActivityTarget): ActivityAvailability {
        ...
        const cost = this.getActivityCost(player, type, target);
        ...
        if (type == ActivityType.Research) {
            ...
            const buildTarget = (target as BuildingActivityTarget);
            const placeable = this.gamedex.placeables[buildTarget.placeableId];
            // Si tiene una tecnología asociada debe estar investigada
            if(placeable.requiredTech){
                const requiredTech = this.gamedex.technologies[placeable.requiredTech];
                if(!researchedTechs.some( tech => tech.id == requiredTech?.id)){
                    failedPreconditions.push('Se necesita '+requiredTech?.media.name+' para llevar a cabo esta actividad');
                }
            }
    ...
    private calculatePlayerProperties(player: InstancePlayer, include?: PropertyCalculationParams): Properties {
        ...
        include = include || { cells: true, placeables: true,technologies: true };
        ...
        if (include.cells) {
            player.cells.map(cellId => this.instance.cells[cellId]).forEach(cellInstance => {
                ...
                if(include.placeables){
                    for(const pInstance of cellInstance.placeables){
                        const placeable = this.gamedex.placeables[pInstance.placeableId];
                        if(pInstance.built){
                            // No se tienen en cuenta los edificios no construidos
        ...
        // A la atención de quien lea esto:
        // Inicialmente las propiedades del jugador se iban a determinar
        // teniendo en cuenta las propiedades de sus recursos. Sin embargo
        // dada la velocidad a la que estos cambian, el intercambio de mensajes
        // de actualización de estado se convierte en una locura. En consecuencia,
        // se desactiva el cómputo de los almacenes.
        //
    ...
    /**
     * Actualiza la cache de propiedades del jugador y notifica
     * a este en caso de encontrar un cambio entre las viejas y
     * las nuevas
     * @param player Jugador cuyas propiedades se calculan
     */
    private updatePlayerProps(player:InstancePlayer):Properties{
        const newProps = this.calculatePlayerProperties(player);
        const oldProps = this.propCache[player.playerId] || {}; // Los nuevos jugadores no tienen nada!
        const diffProps : Record<string,number> = {};

        /* Hay algo en las nuevas que no tengan las viejas? */
        for(const k in newProps){
            if(oldProps[k] == undefined || oldProps[k] != newProps[k]) diffProps[k] = newProps[k];
        }
        /* Ha habido cambios? */
        if(Object.keys(diffProps).length > 0){
            this.sendEvent(player,GameEvents.PlayerPropertiesUpdated,newProps);
            // Ademas, ¿es una propiedad singular?
            if(diffProps[ConstantProperties.InfluenceRadius] != undefined){
                this.sendEvent(player,GameEvents.InfluenceRadiusUpdated,diffProps[ConstantProperties.InfluenceRadius]);
            }
        }

        // Actualizamos la cache y terminamos
        this.propCache[player.playerId] = newProps;
    ...
    private processQueue(): void {
        ...
        this.instance.players.forEach(player => {
            const props = this.updatePlayerProps(player);
            ...
            while (i < queue.length) {
                const item = queue[i];
                const cost = activityCostCalculator(props,this.gamedex,item.type,item.target);

                if (item.elapsed >= cost.time) {
                    ...
                    if (i < queueNumProcesses) {
                        ...
                        item.remaining = cost.time - item.elapsed;
                    } else {
    ...
    private attackPlayer(player: InstancePlayer, target: AttackActivityTarget) {
        ...
        const attackerProps = this.calculatePlayerProperties(attacker, { cells: false, placeables: false, technologies: true });
    ...
    getCells(player: InstancePlayer): CellInstance[] {
        ...
        const unknownCell = this.gamedex.cells[this.game.config.unknownCellId];
    ...
    private createInitialStockpile(): Stockpile[] {
        const stockpiles: Stockpile[] = [];
        
        // Se inician los almacenes con todos los recursos a cero salvo los predefinidos
        this.game.resources.forEach( res => {
            const defaultSp = this.game.defaultPlayerStockpiles.find( dpsp => dpsp.resourceId == res.id);
            let amount = defaultSp?.amount || 0;
            stockpiles.push({resourceId:res.id,amount});
        })
